...
export default function SuratIzinMagangForm() {
  ...
  useEffect(() => {
    ...
    const loadProfile = async () => {
      try {
        ...
        // backend mungkin belum tersedia — tidak memunculkan error berat
        if (mounted) setProfile(null);
      } finally {
  ...
  const validateForm = (): boolean => {
    ...
    if (!instansi.trim()) {
      newErrors.instansi = "Instansi tujuan harus diisi";
    }
  ...
  const handleFileChange = (file: File | null) => {
    if (file) {
      if (file.name.match(/\.(doc|docx)$/i)) {
        ...
        setErrors((prev) => ({ ...prev, file: "Format file harus WORD (.doc atau .docx)" }));
      }
    }
  };
    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="w-full max-w-4xl mx-auto">
      <div className="bg-white rounded-lg shadow-lg p-8 md:p-12">
        <form onSubmit={handleSubmit} className="space-y-8">
          {/* PROFILE + INSTANSI TUJUAN di dalam kotak yang sama dengan field lain */}
          <div>
            ...
            <div className="bg-gray-50 border border-gray-200 rounded-lg p-4 mb-4 flex flex-col sm:flex-row sm:items-center gap-4">
          ...
          <div>
            ...
            />
            {errors.keperluan && (
              <motion.p initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} id="keperluan-error" className="mt-2 text-sm text-red-500 flex items-center gap-1">
                <AlertCircle className="w-4 h-4" />
                {errors.keperluan}
              </motion.p>
            )}
          ...
          {/* UPLOAD FILE */}
          <div>
            <label htmlFor="file-upload" className="block text-lg font-bold text-[#0A1C56] mb-3">
              Unggah Surat
            </label>
            ...
            >
              <input ref={fileInputRef} id="file-upload" type="file" accept=".doc,.docx" onChange={handleFileInputChange} className="hidden" />
              ...
              {!selectedFile ? (
                <label className="flex flex-col items-center justify-center py-10 px-4 cursor-pointer transition-colors">
                  ...
                  <p className="text-sm font-medium text-gray-700 mb-1">Tarik dan Lepas</p>
                  <p className="text-xs text-gray-500">atau klik untuk memilih file (.doc / .docx)</p>
                </label>
              ) : (
                <div className="flex items-center justify-between p-4">
                  ...
                  <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} type="button" onClick={() => setSelectedFile(null)} className="p-1 hover:bg-red-100 rounded-full transition-colors">
            ...
            {errors.file && (
              <motion.p initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} id="file-error" className="mt-2 text-sm text-red-500 flex items-center gap-1">
                <AlertCircle className="w-4 h-4" />
                {errors.file}
              </motion.p>
            )}
          ...
          {/* Tombol Submit */}
          <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} type="submit" disabled={isSubmitting} className={`flex items-center gap-2 px-8 py-3 rounded-lg font-semibold text-white shadow-lg transition-all ${isSubmitting ? "bg-gray-400 cursor-not-allowed" : "bg-[#0A1C56] hover:bg-[#1976D2]"}`}>
            {isSubmitting ? (
              <>
